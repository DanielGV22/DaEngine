...
using namespace DaEngine:: Graphics;
...
{  
   LRESULT GraphicsSystem::GraphicsSystemMessageHandler(HWND hwnd, UINT msg, WPARAM wParam, LPARAM lParam)  
   {  
       if (sGraphicsSystem != nullptr)
       {
           ...
           case WM_SIZE:
           {
               ...
               sGrahicsSystem->Resize(width, height);
               break;
           }
           }
       }
   ...
   {  
       ASSERT(sGraphicsSystem != nullptr, "[GraphicsSystem] is not installed");
   ...
   {  
	   ASSERT(mD3DDevice == nullptr, "[GraphicsSystem] is not terminated");
   }  
   ...
   void GraphicsSystem::SetClearColor()  
   {  
	   mClearColor = color;
   }  
   ...
   uint32_t GraphicsSystem::GetBufferWidth() const  
   {  
	   return mSwapChainDesc.BufferDesc.Width;
   }  

   uint32_t GraphicsSystem::GetBufferHeight() const  
   {  
   ...
   {  
	   return static_cast<float>(GetBackbackBufferWidth()) / static_cast<float>(GetBackBufferHeight());

   }  

   ID3D11Device* GraphicsSystem::GetDevice() const  
   {  
       return nullptr;  
   }  

   ID3D11DeviceContext* GraphicsSystem::GetContext() const  
   {  
       return nullptr;  
   }  
}
